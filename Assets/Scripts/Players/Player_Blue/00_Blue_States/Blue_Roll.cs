...
public class Blue_Roll : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            moveVelocity = new Vector3(Input.GetAxisRaw("Horizontal"), 0f, Input.GetAxisRaw("Vertical")) * 24;
        }

        if (bluePlayerController.useController == false)
        {
            moveVelocity = new Vector3(Input.GetAxisRaw("Key_X"), 0f, Input.GetAxisRaw("Key_Y")) * 24;
        }
