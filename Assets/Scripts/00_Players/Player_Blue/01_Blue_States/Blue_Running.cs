...
public class Blue_Running : MonoBehaviour
{
    ...
    void Update() // RUNNING TO IDLE, Movement, Firing, and Aiming // Update is called once per frame
    {
        ...
        {
            if ((moveInput.x > -.5) & (moveInput.x < .5) & (moveInput.z < 0))
            {
                Debug.Log("roll_0");
                spriteAnimator.SetInteger("rollDirection", 0);
            }

            if ((moveInput.x > 0) & (moveInput.z < .5) & (moveInput.z > -.5))
            {
                Debug.Log("roll_2");
                spriteAnimator.SetInteger("rollDirection", 2);
            }

            if ((moveInput.x > -.5) & (moveInput.x < .5) & (moveInput.z > 0))
            {
                Debug.Log("roll_4");
                spriteAnimator.SetInteger("rollDirection", 4);
            }

            if ((moveInput.x < 0) & (moveInput.z < .5) & (moveInput.z > -.5))
            {
                Debug.Log("roll_6");
                spriteAnimator.SetInteger("rollDirection", 6);
            }
